...
void date2019::plusDay(int dd)
{
	...
	if (check3)
	{
		int originalDay; // prints the date after the specified number of days on screen
		originalDay = (subVar + 1) % 7;
		...
		cout << endl
			<< days << " days after " << dayOfWeek[originalDay] << ", " << monthOfYear[subMon - 1] << " " << subDay << ", 2019 is ";
	}
	else
	{
		exit(0);
...
void date2019::print() //method to print the date in format day, month date, year
{
	if (day != -1 && month != -1)
	{
		...
		cout << dayOfWeek[originalDay] << ", " << monthOfYear[mon - 1] << " " << day << ", 2019" << endl;
	}
	else
	{
		cout 
